  ...
  useServer(
    {
      ...
      subscribe,
      context: async () => {
        if (typeof options.context === 'function') return options.context();
      ...
      onSubscribe: async (_, message) => {
        ...
        if (validationErrors.length > 0) return validationErrors; // return `GraphQLError[]` to send `ErrorMessage` and stop Subscription
